...
pairs = zip(input_train, output_train)

pairs_test = zip(input_dev, output_dev)

corpus, corpus_name, datafile = None, "whatever", extraction_path + ".wtv"

data_to_file(pairs, datafile)
...
save_dir = os.path.join("weel/data", "save")
voc, pairs = loadPrepareData(corpus, corpus_name, datafile, save_dir)



def indexesFromSentence(voc, sentence, to_split=True):
    if to_split:
...
def inputVar(l, voc):
    indexes_batch = [indexesFromSentence(voc, sentence, to_split=False) for sentence in l]
...
if MAKE_MODEL :
    ...
    for epoch in map(str, range(1, EPOCHS + 1)):
        ...
        param_prefix = "batching_test__lr" + str(LEARNING_RATE) +\
            "_d" + str(DROPOUT) +\
            "_e" + epoch +\
            "_"
        if RETRAIN :
        ...
        with open(test_result_path, "w") as ostr:
            ...
            predictions, _ = zip(*(
                model.run(*p, batch_size=p[0].size(1))
                for p in (
                    make_batch(ipts, opts, subword_lookup, decoder_lookup, hollistic_lookup)
                    for ipts, opts in (
                        zip(*chunk)
                        for chunk in to_chunks(zip(input_train, output_train))
                    )
                )
            ))
            rlookup = reverse_lookup(decoder_lookup)
            predictions = (pred for preds in predictions for pred in translate(preds, rlookup))
